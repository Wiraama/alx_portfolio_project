...
class Api:
    ...
    def map_view(lat, lon, direction):
        lat = float(lat)
        lon = float(lon)
        ...
        icon_html = f"""
            <div style="transform: rotate({direction}deg); width: 30px; height: 30px; background: url('{url_for('static', filename='images/kl.png')}') no-repeat center; background-size: contain;"></div>
            """

        flight_icon = folium.DivIcon(html=icon_html)
        folium.Marker([lat, lon], popup=f"Flight Location ({direction}°)", icon=flight_icon).add_to(folium_map)
